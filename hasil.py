...
# Cek kolom yang ada di dalam dataset
st.write("Kolom yang ada dalam dataset:", data.columns)

# Fitur yang digunakan
fitur = ['Price', 'Ratings', 'RAM (GB)', 'ROM (GB)', 'Camera (MP)', 'Battery']

# Pastikan kolom dalam 'fitur' ada dalam dataset
missing_columns = [col for col in fitur if col not in data.columns]
if missing_columns:
    ...
    if any([filter_price, filter_rating, filter_ram, filter_rom, filter_camera, filter_battery]):
        ...
        if filter_camera:
            min_camera = st.selectbox("Pilih Kamera Minimal (MP)", sorted(data['Camera (MP)'].unique()))
            data_filtered = data_filtered[data_filtered['Camera (MP)'] >= min_camera]
        ...
        if not data_filtered.empty:
            ...
            st.dataframe(rekomendasi[['Brand', 'Type', 'Price', 'Ratings', 'RAM (GB)', 'ROM (GB)', 'Camera (MP)', 'Battery']])
        else:
